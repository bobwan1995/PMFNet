...
def evaluate_hoi_vcoco(dataset, all_hois, output_dir):
    """Evaluate vcoco human object interaction triplets"""
    logger.info('Evaluate v-coco')
    not_comp = not cfg.TEST.COMPETITION_MODE
    assert dataset.name.startswith('vcoco')

    vcoco_eval = dataset.VCOCO.do_eval(all_hois, dataset.get_roidb(gt=True), plot_save_path=output_dir)

    res_file = os.path.join(
        output_dir, 'hoi_' + dataset.name + '_results'
    )
    if not_comp:
        res_file += '_{}'.format(str(uuid.uuid4()))
    res_file += '.pkl'
    with open(res_file, 'wb') as f:
        pickle.dump(all_hois, f, pickle.HIGHEST_PROTOCOL)
