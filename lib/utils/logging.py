...
def log_stats0(stats, misc_args):
    """Log training statistics to terminal"""
    lines = '\n [Step %d / %d]\t%.19s\n' % (stats['iter'], cfg.SOLVER.MAX_ITER,datetime.datetime.now())
    ...
    if cfg.MODEL.VCOCO_ON:
        lines += "\t" + ", ".join("%s: %.6f" % (k, v) for k, v in stats['hoi_losses'].items()) + "\n"
    print(lines[:-1])  # remove last new line


def log_stats(stats, misc_args):
    """Log training statistics to terminal"""
    lines = '\n{:.19s}:   Step {} / {}'.format(datetime.datetime.now(), stats['iter'], cfg.SOLVER.MAX_ITER)
