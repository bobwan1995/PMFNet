...
def get_fast_rcnn_blob_names(is_training=True):
    ...
    if is_training:
        ...
        # used to for sample human and role-object rois in hoi-branch
        blob_names += ['rois_to_gt_ind_map']
        blob_names += ['rois_max_overlaps']
...
def _sample_rois(roidb, im_scale, batch_idx):
    ...
    # Keep rois(proposal) corresponding gt_inds and max_overlap
    # For sampling samples for human-centric branch and interaction branch
    sampled_rois_to_gt_ind_map = roidb['box_to_gt_ind_map'][keep_inds]
    sampled_rois_max_overlaps = roidb['max_overlaps'][keep_inds]
    ...
    blob_dict = dict(
        ...
        rois_to_gt_ind_map=sampled_rois_to_gt_ind_map,
        rois_max_overlaps=sampled_rois_max_overlaps
    )
